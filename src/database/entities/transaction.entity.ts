...
export class Transaction implements TransactionInterface {
    ...
    status: string
    @ManyToOne(() => User, user => user.sentTransactions, { cascade: true, onDelete: "CASCADE", onUpdate: "CASCADE" })
    @JoinColumn({ name: "sender_id" })
    sender: User;

    @ManyToOne(() => User, user => user.receivedTransactions, { cascade: true, onDelete: "CASCADE", onUpdate: "CASCADE" })
